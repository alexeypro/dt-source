...
/**
 * 
 * Implementation of dtsource:hibernate tag.
 * Works with Hibernate's implementation of Entity manager.
 * Provides Hibernate-source for loading lists.
 * 
 * @author olexiy, white
 *
 */
public class HibernateTag extends BodyTagSupport {

	// Instantiate Logger class
	private static final Logger log = Logger.getLogger(HibernateTag.class);
		
	private static final long serialVersionUID = -521844403928178922L;
	...
	private String entity;			// which entity (mapped POJO) we are going to get in the collection
	private String alias;				// shortname, alias of the entity
	...
	public void setPagesize(Long pagesize) {
		log.debug("setPagesize(Long) called");
		this.pagesize = pagesize;
	}
	
	public void setPagesize(Integer pagesize) {
		log.debug("setPagesize(Integer) called");
		if (pagesize == null)	this.pagesize = (long)0;
		else					this.pagesize = Long.valueOf(pagesize.toString());		
	}
	...
	public int doEndTag() throws JspException {
		...
		EntityManager em = (EntityManager) context.getBean(Constants.ENTITYMANAGER_HIBERNATE_NAME, EntityManager.class);
		if (em == null) {
			throw new JspException("Cannot locate the EntityManager in the WebApplicationContext, under the name: '" + Constants.ENTITYMANAGER_HIBERNATE_NAME + "'");
		}
		if ((this.query.toUpperCase().lastIndexOf(Constants.TAG_ALIAS.toUpperCase()) == -1) ||
			...
			(this.query.toUpperCase().lastIndexOf(Constants.TAG_ENTITY.toUpperCase()) == -1)) {
			throw new JspException("Sorry, but query tags: " + Constants.TAG_ALIAS + ", " + Constants.TAG_SORT + " and " + Constants.TAG_ENTITY + " are required.");
		}
		...
		log.debug("this.pagesize = " + this.pagesize);
		...
		this.query = this.query.replaceAll(Constants.TAG_ALIAS, this.alias).replaceAll(Constants.TAG_ENTITY, this.entity);
		...
		if (useCriteria) {
			...
			String criteriaRule = pu.createCriteriaWhereHQL(this.alias, filterField, filterCriteria);
		...
		querySize = pu.createCountQueryHQL(this.entity, this.alias, completeWhereClause);

        // debug information just for now
		log.debug("[after criteria] HQL query for counting size: " + querySize);
		log.debug("[after criteria] HQL query for fetching list: " + query);
		...
		if (useSearch) {
			...
			String searchRule = pu.createSearchWhereHQL(this.alias, searchField, searchFrom, searchTo);
		...
		// debug information just for now
		log.debug("[after search] HQL query for counting size: " + querySize);
		log.debug("[after search] HQL query for fetching list: " + query);
